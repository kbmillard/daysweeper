  ...
  return (
    <div className="space-y-4 p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <AssignControl routeId={id} currentAssigneeId={route.assignedToUserId} meId={user?.id || undefined} canReassign={canReassign} />
      </div>

      <div className="flex flex-wrap items-center gap-2">
        <form action={`/api/routes/${id}/optimize`} method="post" onSubmit={(e) => { e.preventDefault(); fetch(`/api/routes/${id}/optimize`, { method: "POST" }).then(() => setTimeout(() => window.location.reload(), 350)); }}>
          <Button type="submit" size="sm" variant="secondary" disabled={optimizeRoute.isPending || stopObjs.length < 2}>
            {optimizeRoute.isPending ? "Optimizing..." : "Optimize (Mapbox)"}
          </Button>
        </form>
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={optimizeRoute.isPending || stopObjs.length < 2}
              >
