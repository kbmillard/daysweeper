...
import { VoiceTranscribeField } from '@/components/company-interactions/voice-transcribe-field';
import { TranscriptSnippet } from '@/components/company-interactions/transcript-snippet';
...
export default function CompanyInteractions({ companyId }: Props) {
  ...
  // Voice/transcript and note text only persist after "Add Interaction". Warn if closing mid-edit.
  useEffect(() => {
    if (!showForm || !formData.content.trim()) return;
  ...
  const confirmDeleteInteraction = async () => {
    ...
    try {
      const res = await fetch(
        `/api/companies/${companyId}/interactions/${deleteId}`,
        { method: 'DELETE' }
      );
      if (!res.ok) {
      ...
      setInteractions((prev) => prev.filter((i) => i.id !== deleteId));
  ...
  return (
    <Card>
      <AlertModal
        ...
        description='This interaction will be removed from the database.'
      />
      <CardHeader>
      ...
      <CardContent className='space-y-4'>
        ...
        {!migrationsRequired && showForm && (
          <form onSubmit={handleSubmit} className='space-y-4 p-4 border rounded-lg'>
            ...
            <VoiceTranscribeField
              disabled={migrationsRequired}
              appendToContent={(chunk) =>
                setFormData((fd) => ({
                  ...fd,
                ...
                >
                  ...
                  <div className='flex-1 min-w-0 space-y-1'>
                    <div className='flex items-center gap-2 flex-wrap'>
                      <Badge variant={getTypeColor(interaction.type) as 'default' | 'secondary' | 'outline'}>
                    ...
                    <TranscriptSnippet text={interaction.content} />
                  </div>
                  {!migrationsRequired && (
